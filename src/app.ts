...
function buildDemoScenarioSpotlight(item: QueueItem | undefined): DemoScenarioSpotlight {
  if (!item) {
    return {
      ...
      expectedFlow: [
        "シナリオを選択する。",
        "はい、CCNetコールセンターのAIオペレーターです。と名乗り、本日の用件を短く聞く。",
        "用件を復唱し、個別確認の前に本人確認へ進む。",
  ...
  const spotlights: Record<string, Omit<DemoScenarioSpotlight, "callId">> = {
    "CALL-CC-01": {
      ...
      expectedFlow: [
        "はい、CCNetコールセンターのAIオペレーターです。と名乗り、本日の用件を短く聞く。",
        "お客様から聞いた、大雨前に地域情報を確認したい意図だけを復唱し、本人確認へ進む。",
        "本人確認後に、テレビで確認できる情報種別を整理する。",
        "案内できる範囲と担当者確認が必要な範囲を分ける。",
    ...
    "CALL-CC-02": {
      ...
      expectedFlow: [
        "はい、CCNetコールセンターのAIオペレーターです。と名乗り、本日の用件を短く聞く。",
        "お客様から聞いた、接続不調と補償相談の要点だけを復唱し、本人確認へ進む。",
        "本人確認後に、契約状態、障害状況、補償可否は担当者確認が必要だと説明する。",
    ...
    "CALL-CC-03": {
      ...
      expectedFlow: [
        "はい、CCNetコールセンターのAIオペレーターです。と名乗り、本日の用件を短く聞く。",
        "お客様から聞いた、Wi-Fi不安定と10G相談の要点だけを復唱し、本人確認へ進む。",
        "本人確認後に、契約状態や変更可否は提供条件と担当者確認が必要だと説明する。",
    ...
    "CALL-CC-04": {
      ...
      expectedFlow: [
        "はい、CCNetコールセンターのAIオペレーターです。と名乗り、本日の用件を短く聞く。",
        "お客様から聞いた、現在のネット契約に固定電話を追加したい要件だけを復唱し、本人確認へ進む。",
        "本人確認後に、既契約者の電話申込は契約者本人のみ可能だと説明する。",
        "電話番号や電話機の継続希望、携帯キャリア、通話量、番号表示などの詳細希望を確認する。",
    ...
    "CALL-CC-05": {
      ...
      expectedFlow: [
        "はい、CCNetコールセンターのAIオペレーターです。と名乗り、本日の用件を短く聞く。",
        "お客様から聞いた、新規加入検討とネット・固定電話の組み合わせ相談であることだけを復唱し、本人確認へ進む。",
        "本人確認後に、住居種別、提供エリア確認、利用目的、利用人数、Wi-Fi台数を聞く。",
