...
- Draft 09以降の大きな構想は一部だけ前倒し済みで、回答制御のpolicy guard、代表シナリオ回帰、fallback / rehearsal mode、CCNetのサービス詳細・約款・重要事項説明に合わせた役員デモ仕上げはTask 22までに実装済み。サマリー、通話風UI、Realtime音声はまだ明示的な実装Taskとして残っている。
