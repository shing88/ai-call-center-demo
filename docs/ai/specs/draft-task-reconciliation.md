...
- 実行済みTaskはTask 24 `browser-call-style-ui`まで。
- Draft 02〜05は、おおむね同番号の実行Taskとして完了している。
- Draft 06以降は、ドラフトをそのまま順番に実装せず、現在のデモに合わせて根拠表示、会話プレビュー、AI response境界へ再分割した。
- Draft 09以降の大きな構想は一部だけ前倒し済みで、回答制御のpolicy guard、代表シナリオ回帰、fallback / rehearsal mode、CCNetのサービス詳細・約款・重要事項説明に合わせた役員デモ仕上げ、ローカル決定的な応対サマリー、役員に見せやすい通話風UIはTask 24までに実装済み。Realtime音声とtool callingはまだ本番接続としては未実装。
- 次の実装はTask 25 `realtime-api-connection-boundary`で、公式ドキュメント確認、設定境界、未接続UI、安全監査を先に固める。
...
| Draft 12 `realtime_api_connection` | Realtime API接続、マイク入力、AI音声応答 | Task 14〜16でAPI接続前のrequest/client/network境界だけ実装。Task 25で接続境界と未接続UIを扱う | 本番Realtime接続は未実装 |
...
Realtime APIへ進む前に、公式ドキュメント確認、API key / token / mic permission / external send境界、未接続UI、local fallback維持を固める。
