...
- 実行済みTaskはTask 20 `demo-scenario-regression-suite`まで。
- Draft 02〜05は、おおむね同番号の実行Taskとして完了している。
- Draft 06以降は、ドラフトをそのまま順番に実装せず、現在のデモに合わせて根拠表示、会話プレビュー、AI response境界へ再分割した。
- Draft 09以降の大きな構想は一部だけ前倒し済みで、回答制御のpolicy guardと代表シナリオ回帰はTask 20までに実装済み。サマリー、通話風UI、Realtime音声、fallback、役員デモ仕上げはまだ明示的な実装Taskとして残っている。
- 次の実装はTask 21 `fallback-rehearsal-mode`で、外部AI API、音声、通信が使えない場合でもデモを継続できる状態を作る。
...
| Draft 15 `fallback_rehearsal_mode` | 通信失敗時のfallback / rehearsal | Task 21で現在の静的TypeScriptデモに合わせて実装予定 | 次 |
...
### Task 21: fallback-rehearsal-mode
