...
現在のPR段階: Live Realtime failure diagnostics and browser-mic handoff
...
後続計画: `OPENAI_API_KEY`をGit管理外の`.env.local`で用意したDocker環境では、`GET /api/health`が`configured` / `ready`を返し、`POST /api/realtime/client-secret`が`HTTP 200` / `status=ready` / `valueあり`を返すところまで確認済み。Codex in-app browserでは`GET /api/health`反映後に`Token endpoint configured`が表示され、`Start call`後のfallback診断で`Stage: microphone` / `NotFoundError`が表示されることを確認済み。ユーザー実機ブラウザでは`Start call`後に`Realtime unavailable, using fallback rehearsal`へ戻ったため、実Realtime音声接続完了はまだ主張しない。現在はfallback時の失敗ステージ、HTTP status、microphone stateを画面に残し、`GET /api/health`のconfigured状態をRealtime boundaryへ反映する小PRを進める。次は実マイクを許可できる通常ブラウザで、接続状態または失敗ステージを見たらすぐ`End call`または停止する短時間確認を行う。keyがない環境では`OPENAI_API_KEY`なしのfallback表示、secret非露出、既存テストだけを確認する。
