...
- Assistant handoffにはbuild時に生成した根拠候補manifestの内容、デモ用応答ドラフト、会話履歴風プレビューが表示され、キュー項目の選択に応じて該当call idの根拠候補、ドラフト、プレビューへ切り替わる。
...
- `src/app.ts`は`AssistantEvidence`表示用データを受け取り、出典、section、snippet、scoreをAssistant handoffに表示し、選択中キューと根拠候補からデモ用応答ドラフトと会話履歴風プレビューを決定的に作る。
...
- `src/main.ts`はmanifest取得に成功した場合はその根拠候補を表示し、キュー項目の「開く」操作で該当bundleをAssistant handoffへ反映する。表示再描画時に応答ドラフトと会話履歴風プレビューも選択中call idへ追従する。manifest取得失敗時や該当bundleなしの場合は既存表示を維持する。
...
- `npm test`はアプリ描画ロジック、キュー選択状態、Assistant handoffの根拠候補表示、デモ用応答ドラフト、会話履歴風プレビュー、evidence manifest/fallback、knowledge Markdown baselineの構造、Markdown loader / chunk model、keyword search / 根拠候補抽出、evidence bridgeを検査する。
...
- 次の実装タスクは未定。候補は会話履歴風プレビューへのユーザー入力導線、検索ランキング高度化、または外部AI API接続準備だが、実行前に`docs/ai/tasks/`配下の短いタスク指示として定義する。
...
- Task 11 `conversation-thread-preview`は完了。
- 次は会話履歴風プレビューへのユーザー入力導線、検索ランキング高度化、または外部AI API接続準備を進める実行可能タスク指示を作成してから着手する。
