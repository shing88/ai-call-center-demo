...
- `End call`後はdata channel `oai-events`で受け取ったRealtime transcript eventをbrowser stateのhandoff recordへまとめ、summary、evidence references、policy decision、next actionと一緒に画面へ残す。永続保存・外部送信・実電話接続は引き続きblocked。
...
- 実電話、DB保存、認証、本番接続、transcript/summaryの永続保存は未実装。Realtime音声はブラウザの`Start call`から短命client secretで接続するデモ境界のみ。Realtime handoff recordとOperator noteはbrowser-onlyの未送信・未保存値。
...
- `src/realtime-call-recording.ts`: Realtime transcript event collector、browser-only handoff record、summary/evidence/policy/next actionの記録境界。
...
- `src/main.ts`: manifest取得、キュー選択、Operator noteのブラウザ内メモリ保持、Realtime call controlsのクリック処理、選択中callのRealtime grounding request生成、Realtime handoff record生成、再描画。
...
- Task 28 `browser-realtime-voice-demo`のCall recording and handoffをPR化する。
- 次のTask 28候補は、必要になった場合だけDocker再起動後も残すSQLite/local JSON persistenceを小さく追加する。実電話接続、認証、本番DB、外部送信は引き続き対象外。
