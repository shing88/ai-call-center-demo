...
- `src/realtime-call-controls.ts`: browser-safeな`Start call` / `End call`状態、OpenAI Realtime WebRTC calls endpoint、ephemeral client secret取得、mic permission、SDP offer/answer、data channel server event受信、fallback復帰、失敗ステージ診断、local track/peer connection cleanup。
...
- Codex in-app browserでは`GET /api/health`反映後に`Token endpoint configured`が表示され、`Start call`後のfallback診断で`Stage: microphone` / `NotFoundError`が表示されることを確認済み。ユーザー実機ブラウザでは`Start call`後に`Realtime unavailable, using fallback rehearsal`へ戻っており、実Realtime音声接続完了までは確認できていない。次回の実機確認ではfallback時の失敗ステージ、HTTP status、microphone stateを画面で読める。
