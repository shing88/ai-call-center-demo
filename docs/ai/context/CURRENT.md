...
- keyがない環境では実Realtime接続成功を主張しない。`OPENAI_API_KEY`なしのfallback表示、secret非露出、既存124件のテストだけを確認する。keyあり環境では短時間の実機ブラウザ確認で`Realtime call connected`、`End call`、handoff復元まで確認済み。
